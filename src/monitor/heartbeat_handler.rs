...
fn _run_main_loop(rx: Receiver<HeartbeatRequest>) {
    ...
    let heartbeat_monitor_client = mqtt_utils::get_client();
        ...
        if let HeartbeatRequestType::Update = received.request_type {
            ...
            if dead_nodes.contains_key(&server_number) {
                ...
                send_heartbeat_message(
                    ...
                    &server_number, // Send id of new server
                )
            }
        ...
        if let HeartbeatRequestType::SearchDeadNodes = received.request_type {
            ...
            for (node_id, last_heartbeat) in &last_heartbeats {
                if now - last_heartbeat > MAX_HEARTBEAT_INTERVAL {
                    ...
                    thread::sleep(Duration::from_millis(4000)); // Simulate startup
...
pub fn send_heartbeat_message(heartbeat_monitor_client: &Client, req_type: &String, value: &i32) {
